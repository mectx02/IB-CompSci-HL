package HL_1.Section_08.AP_Free_Response.GroceryList;
...
public class GroceryStore
{
	...
	public String bargainItem(String category)
	{
            ...
            for (int i = 0; i < numItems; i++){
                ...
                (item.getItemPrice() / item.getItemSize()) < lowestPrice) {
                    
                    lowestPrice = item.getItemPrice();
                    lowestName = item.getItemName();
                    
                } else if (item.getItemCategory().equals(category) &&
                (item.getItemPrice() / item.getItemSize()) == lowestPrice)
                    lowestName += " and " + item.getItemName();
